            ...
            {metricData.map(data => (
                    <div key={data.name} className="bg-white rounded-lg p-4">
                        <section className={"flex items-center mb-3"}>
                            ...
                            <p>{data.name}</p>
                            <div className={`${data.type==="normal"?"bg-[#f0fdf4]":"bg-red-100"} ms-auto mb-auto p-1 rounded-lg text-sm`}   >
                                <p>{data.type}</p>
                            </div>
                        </section>
                        <section className={"flex"}>
                            <p className={"text-xl font-bold me-2"}>{data.metrics}</p>
                            <p className={"mt-auto text-sm"}>{data.metricType}</p>
                        </section>
